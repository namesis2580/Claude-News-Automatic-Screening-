...
description: >-
  Sets up Claude Code SessionStart hooks for claude.ai web platform sessions.
  TRIGGER when user wants to: configure .claude/hooks/session-start.sh,
  auto-install project dependencies (npm install / pip install / cargo build)
  when a Claude Code web session starts, or make tests/linters work in Claude
  Code on the web.
  DO NOT TRIGGER for: git hooks (pre-commit, pre-push, post-receive),
  CI/CD pipelines (GitHub Actions, Jenkins), Docker setup, Makefile targets,
  general bash scripting, or Slack/HTTP webhooks unrelated to Claude Code
  web sessions.
---
